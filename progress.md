...
- 2026-06-20 已实现盈亏纳入赎回(REDEEM)+ 用真实 usdcSize:预测市场仓位会「持有到结算按 $1/$0 赎回」离场,/trades 不含赎回,导致"买了没卖、结算输掉"的亏损被漏算(平均成本法只认卖出),已实现虚高。改用 `/activity`(TRADE+REDEEM),`getActivity` 返回 {trades,redeems};`computeTradeHistory` 按时间合并回放,REDEEM 按 conditionId 清账剩余持仓 realize=赎回所得−剩余成本基;金额改用真实 `usdcSize`(非 size×price)。实测某账户已实现从虚高 $5.15 → 真实 $3.57(= 现金口径 卖+赎−买 $3.56);残差 $0.77 = maker 奖励/入金>$10。UI 加 REDEEM 行(绿色 chip)。sanity(held-to-win/loss/partial)+ build 通过。
- 2026-06-20 用户反馈修正:① #4 时间提示数据源彻底重做(gamma 无比赛结束字段,endDate==gameStartTime==开赛 → 体育=开赛倒计时/开赛后「进行中」,聚合=结算倒计时);② #1 sparkline 不再挤占现价(数字块不收缩);③ #5/#7 卡片改无缝通栏;④ 止损跌% 上限 50→150→100%;⑤ **#2 成交流水补 maker 成交**(data-api 默认 takerOnly=true 漏掉用户挂单成交的 maker 部分,买卖皆受影响 → 改 takerOnly=false 取全);⑥ **#2 计入 Polymarket 2026 手续费**:卖单/maker 免、仅 buy taker 收费,fee=feeRate×量×价×(1−价)(峰值 50¢;feeRate=每100股峰值费/25:体育0.03/政治0.04/加密0.072,可在设置改默认 0.03)。getTrades 两次拉取(takerOnly true/false)diff 标记每笔 isTaker;computeTradeHistory 把 taker 买入费并入成本基,totalRealized 即净值,另出 buyTakerFees。离线 sanity(官方峰值对齐 + 成本基整合 + maker 免)+ 真实数据(7 taker/4 maker 买入正确区分)+ build 通过。⚠ 费率公式由官方各品类「每100股峰值费」反推对齐(.com 口径;.us 不同),Chrome 实测待。
- 2026-06-20 手续费精确版(订正上条):上条用「每100股峰值费/25」反推的费率与公式被**官方权威数据订正**——gamma 的 market 对象直接暴露 `feeSchedule {rate, exponent, takerOnly, rebateRate}` + `feesEnabled`,按市场、自动适配品类与未来调价。正确公式(官方 help center worked example 100股@50¢=$0.375)为 `fee = rate×量×价×(价×(1−价))^exponent`——比反推版**多一个 ×价**,峰值在 ~2/3 而非 50¢。改:gammaApi `MarketMeta` 加 `fee`(解析 feeSchedule)、`getMarketMeta` 取 **open+closed 合并**(gamma 默认排除已结算,历史成交多在已结算市场);`tradeHistory` 用 `FeeResolver` 按 conditionId 取官方费率;移除手动 `config.takerFeeRate`/设置项。实时性零影响(仅流水弹窗按需拉)。真实数据 9/9 市场命中 `rate=0.03`。
...
## 止损系统抗插针重构(2026-06-20,已合并 main + push origin)

**触发**:用户「武装的止损被插针打掉,本能赚钱的仓只回本」。诊断旧止损(P4)三大病根:**单档买一报价 + 单 tick 即触发 + 从峰值算回撤**——三个最不抗插针的选择全占了;且无日志系统(无法复盘)。

**设计**(完整 spec/原理/四例见 `stop_loss_redesign_plan.md`,经多轮与用户讨论敲定):
- **共享原语**:中位数参考价(`medianRef`,滤单针)、`DwellGate`(持续确认)。
- **detector 重写**:激活式跟踪 + **保本地板**(盈利仓最坏保本)、`maxLossPct` 下行地板、价位缩放阈值、**低价退化**(<0.10 走绝对地板)、定长 dwell;三种锚点模式(激活跟踪/固定离场/峰值跟踪)。
- **Phase 2a**:波动自适应阈值(时间感知 EWMA,**用上一拍环境波动算本拍阈值**,分离环境/瞬时)、**速度自适应 dwell**(深破确认更快,取代独立灾难线)。
- **Phase 2b**:**半自动确认卡片**(opt-in;触发弹卡片+倒计时+通知,10s fail-safe,取消=snooze 30s)。
- **UI**:精简面板(模式上下文滑块 + 高级覆盖/↺跟随全局)+ **实时止损线读出** + 全局默认设置区;`previewStopLoss` 草稿预览;PositionRow 武装横幅改读实时状态(原读已废字段显示全 0)。
- **明确不做**:成交确认 requireTradeConfirm(需接实时成交流,ROI 低,默认 false)、深度加权参考(opt-in 再说)、大单分批(用户不涉及大单)、MEDIUM5 旧字段迁移(won't-fix)。

**流程**:CLAUDE 规划 → 沙盒限制致 Codex 写不了 sibling worktree,**改由 Claude 实现** + **Codex 只读交叉验证**(多轮),Claude 逐条核实(不照单全收)。

**交叉验证修掉的真问题**(Codex 抓 + Claude 核实):
- Phase 1:SEVERE OCO 过时中位数触发、空盘帧不打断 dwell(且会清空 peak/激活)、全局默认未注入运行时。
- Phase 2a:SEVERE **EWMA 更新顺序**(本拍位移先污染再用→长间隔单根大跌漏判)→改用上一拍 σ;预览配置漏未暴露字段会漂移。
- Phase 2b:SEVERE **FAK 零成交不重试**(急跌零成交被误判已保护、冷却 60s 静默失效)→store+后台双层按成交量判、零成交 settle/短退避重试;取消半自动永久静默→改 snooze;fail-safe 加逐 tick 兜底。

**测试**:逻辑层 14 项单测(detector/monitor/exit,含插针/激活/低价/速度/EWMA 顺序/settle 重试 + 四个贯穿例子);typecheck+build 全绿。

**结果**:6 个 feature/fix 提交 + HTML 实盘指导,合并 `main`(merge `d076682`,HTML `a5b5247`)并 `push origin`;worktree 与临时分支已清理。**实盘验收待用户**(指导 `stop_loss_live_verification.html`,按 dryRun→最小额→放大)。⚠ 关键约束:面板关闭=无保护、急跌可零成交、单点必死/结算归零救不了、无日志系统。
